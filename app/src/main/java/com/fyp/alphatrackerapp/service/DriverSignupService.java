...
public interface DriverSignupService {
    ...
    @Multipart
    @POST(EndPoint.CREATE_DRIVER_ACCOUNT_URL)
    Call<User> user_account(
            ...
            @Part("user_end_lat") RequestBody user_end_lat,
            @Part("user_end_lng") RequestBody user_end_lng,
            @Part("user_gender") RequestBody user_gender,
            @Part("user_routeID") RequestBody user_routeID,
            @Part("user_vehicleID") RequestBody user_vehicleID,
